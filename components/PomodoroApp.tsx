  ...
  const renderMeta = () => (
    <div className="grid gap-4 md:grid-cols-2">
      <div className="p-6 text-sm rounded-3xl border border-white/10 bg-white/5 backdrop-blur-xl shadow-lg text-white/80 transition-all hover:bg-white/10">
        ...
        <p className="mt-2 font-mono text-xs break-all text-white/80">
          {pomodoroAddress}
        </p>
        <div className="mt-6">
          <p className="text-xs font-medium uppercase tracking-widest text-white/40">
          ...
          <p className="text-3xl font-bold text-white tracking-tight mt-1">
            ...
            <span className="text-lg font-normal text-white/60">C2FLR</span>
          </p>
        </div>
      </div>
      ...
      {state.hash && (
        <div className="p-6 text-sm rounded-3xl border border-sky-500/20 bg-sky-500/5 backdrop-blur-xl shadow-lg transition-all hover:bg-sky-500/10">
          <p className="text-xs font-medium uppercase tracking-widest text-sky-300/60">
            Last Transaction
          </p>
          <p className="mt-2 font-mono text-xs text-sky-100/80 break-all leading-relaxed">
            {state.hash}
          </p>
        </div>
      )}
      ...
      {state.error && (
        <div className="p-6 text-sm rounded-3xl border border-rose-500/20 bg-rose-500/5 backdrop-blur-xl shadow-lg">
          <p className="text-xs font-medium uppercase tracking-widest text-rose-300/60">
            Error
          </p>
          <p className="mt-2 text-rose-100/90 leading-relaxed">
            {state.error.message}
          </p>
        </div>
      )}
      ...
      <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-white/60 max-w-md leading-relaxed">
            Simulate a successful session completion to verify rewards. This
            interacts with the contract exactly like a real session end.
          </p>
        </div>
        <button
        ...
        >
          ...
          <span className="relative z-10">
            {state.isLoading ? "Processing..." : "Trigger Completion"}
          </span>
        </button>
      </div>
    </div>
        ...
        <section className="relative overflow-hidden p-12 rounded-[2.5rem] border border-white/10 bg-white/5 backdrop-blur-2xl shadow-2xl">
          ...
          <div className="relative z-10">
            ...
            <p className="max-w-2xl text-lg text-white/60 leading-relaxed mb-10">
              Stake C2FLR to commit to your work. Complete the 25-minute session
              to earn your stake back plus rewards. Break the chain, lose the
              stake.
            </p>
            ...
            <div className="grid gap-4 md:grid-cols-3">
              {[
                ...
                <div
                  key={item.title}
                  className="p-6 rounded-2xl border border-white/5 bg-white/5 backdrop-blur-md hover:bg-white/10 transition-colors"
                >
            ...
            <div>
              ...
              <p className="text-lg text-white/60">
                ...
                <span className="text-white font-semibold">
                  {(stakeValue * 1.1).toFixed(4)} C2FLR
                </span>
              </p>
            </div>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <div className="relative p-10 flex items-center justify-center rounded-[2.5rem] border border-white/10 bg-white/5 backdrop-blur-xl shadow-xl min-h-[400px]">
            <div className="relative w-72 h-72">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                ...
                <span className="text-sm font-medium text-white/40 mt-2 uppercase tracking-widest">
                  Focus
                </span>
              </div>
            </div>
          ...
          <div className="p-10 flex flex-col justify-between rounded-[2.5rem] border border-white/10 bg-white/5 backdrop-blur-xl shadow-xl">
            <div className="grid grid-cols-2 gap-4 mb-8">
              <Stat
                label="Staked Amount"
                value={`${data.session.stakeAmount} C2FLR`}
              />
              <Stat
                label="Bonus Reward"
                value={`+${(stakeValue * 0.1).toFixed(4)} C2FLR`}
              />
              <Stat
                label="Status"
                value={state.isLoading ? "Syncing..." : "In Progress"}
              />
            ...
            <div className="space-y-4">
              ...
              >
                {remaining > 0
                  ? `Complete in ${formatTime(remaining)}`
                  : "Claim Victory"}
              </button>
        ...
        <section className="relative p-12 rounded-[2.5rem] border border-emerald-500/30 bg-emerald-900/20 backdrop-blur-2xl shadow-[0_0_50px_rgba(16,185,129,0.1)] text-center overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <p className="text-xl text-emerald-100/80 mb-10">
              You stayed focused. Here is your reward.
            </p>
            ...
            <div className="grid gap-4 max-w-2xl mx-auto mb-10 md:grid-cols-3">
              ...
              <RewardStat
                label="Bonus"
                value={`+${(stakeValue * 0.1).toFixed(4)}`}
              />
              <RewardStat
                label="Total Payout"
                value={`${(stakeValue * 1.1).toFixed(4)}`}
                highlight
              />
            </div>
      ...
      <section className="relative p-12 rounded-[2.5rem] border border-white/10 bg-white/5 backdrop-blur-2xl shadow-2xl overflow-hidden">
        ...
        <div className="relative z-10 max-w-2xl">
          ...
          <p className="text-lg text-white/60 leading-relaxed">
            Set your stake. If you complete the 25-minute session, you get it
            back plus a bonus. If you quit, you lose it. Simple accountability.
          </p>
        </div>
      </section>
      ...
      <div className="grid gap-6 lg:grid-cols-[1.5fr_1fr]">
        <div className="p-10 rounded-[2.5rem] border border-white/10 bg-white/5 backdrop-blur-xl shadow-xl">
          ...
          <div className="space-y-8">
            <div>
              <label
                className="block text-xs font-bold uppercase tracking-widest text-white/40 mb-3"
                htmlFor="stake"
              >
                Stake Amount (C2FLR)
              </label>
              ...
              <p className="mt-3 text-xs text-white/30 pl-2">
                Minimum stake: 0.001 C2FLR
              </p>
            </div>
        ...
        <div className="p-10 rounded-[2.5rem] border border-white/10 bg-white/5 backdrop-blur-xl shadow-xl flex flex-col justify-center">
          ...
          <div className="space-y-6">
            {[
              ...
              {
                step: "02",
                text: "Focus for 25 minutes without interruption.",
              },
          ...
          <div className="mt-8 p-5 rounded-2xl bg-indigo-500/10 border border-indigo-500/20">
            <p className="text-xs text-indigo-200 leading-relaxed">
              <span className="font-bold">Note:</span> Leaving the page or
              forfeiting will result in the loss of your staked amount to the
              community pool.
            </p>
          </div>
        </div>
  ...
  >
    <p
      className={`text-xs font-bold uppercase tracking-widest mb-2 ${
        highlight ? "text-emerald-200" : "text-white/40"
      }`}
    >
      {label}
    </p>
